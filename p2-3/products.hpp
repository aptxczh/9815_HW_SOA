...
enum Future_Underlying_Type { EuroDollarFuture, BondFuture }; // for now, we consider only two types of futures
enum Future_Settlement_Type{Physical, Cash}; // Futures can be settled either physical or in cash
class Future :public Product {
	//Class representing futures. Basic types include EuroDollarFuture and BondFuture; Physical Settlement and Cash 
	//Settlement. Inherited by subclasses EuroDollarFuture and BondFuture
public:
	// Future ctor
	Future(string _productId, Future_Underlying_Type _futureUnderlyingType, 
		Future_Settlement_Type _futureSettlementType, string _ticker, date _maturityDate);
	Future();

	// Return the maturity date of the future
	date GetMaturityDate() const;
	// Return the settlement tpe of the future
	Future_Settlement_Type GetfutureSettlementType() const;
	// Return the future underlying asset type
	Future_Underlying_Type GetfutureUnderlyingType() const;
	// Return the future's ticker
	string GetTicker() const;
	// Overload the << operator to print out the future
	friend ostream& operator<<(ostream &output, const Future &future);
	virtual void print();
private:
	Future_Underlying_Type futureUnderlyingType; // future Underlying Type type variable
	Future_Settlement_Type futureSettlementType;// future settlement type variable
	string ticker;
	date maturityDate; // maturity date variable
					   // return a string represenation for the payment frequency
protected:
	string ToString(Future_Underlying_Type fut) const;

	// return a string representation for the floating index
	string ToString(Future_Settlement_Type fst) const;
};

class EDFuture :public Future {
public:
	EDFuture(string _productId, string _issue_country, string _ticker, date _maturityDate);//ctor
	EDFuture(){}//default ctor
	virtual void print();//print out message
private:
	string issue_country;
	//indicates the country which issued the future
};
class BDFuture :public Future {
public:
	BDFuture(string _productId, Bond _bd, string _ticker, date _maturityDate);//ctor
	BDFuture() {}//default ctor
	virtual void print();//print out message
private:
	Bond bd;//Underlying bond
};
...
///////////////////////////////
Future::Future(string _productId, Future_Underlying_Type _futureUnderlyingType,
	Future_Settlement_Type _futureSettlementType, string _ticker, date _maturityDate)
	:Product(_productId, FUTURE)
{//Constructor
	futureUnderlyingType = _futureUnderlyingType;
	futureSettlementType = _futureSettlementType;
	maturityDate = _maturityDate;
	ticker = _ticker;
}

Future::Future() :Product(0, FUTURE) {}
Future_Underlying_Type Future::GetfutureUnderlyingType() const {//return underlying type
	return futureUnderlyingType;
}
date Future::GetMaturityDate() const {//return MaturityDate
	return maturityDate;
}
Future_Settlement_Type Future::GetfutureSettlementType() const {//return settlement type
	return futureSettlementType;
}

string Future::GetTicker() const {//return ticker
	return ticker;
}
string Future::ToString(Future_Underlying_Type fut) const {
	switch (fut)
	{
	case BondFuture:return "BondFuture";
	case EuroDollarFuture:return "EuroDollarFuture";
	default:
		return "";
	}
}
...
// return a string representation for the floating index
string Future::ToString(Future_Settlement_Type fst) const {
	switch (fst)
	{
	case Cash:return "Cash Settled";
	case Physical:return "Physical Settled";
	default:
		return "";
	}
}
...
{
	output << future.ToString(future.GetfutureUnderlyingType()) << ", "<<future.ToString(future.GetfutureSettlementType())<<
		", Ticker: " << future.GetTicker() << ", Maturity: " << future.GetMaturityDate();
	return output;
}
void Future::print() {
	cout << *this<<endl;
...
EDFuture::EDFuture(string _productId, string _issue_country, string _ticker, date _maturityDate) :
	Future(_productId, EuroDollarFuture, Cash, _ticker, _maturityDate) {
	issue_country = _issue_country;
}
void EDFuture::print() {//Polymorphically print information of future
	cout << this->ToString(this->GetfutureUnderlyingType()) << ", " << this->ToString(this->GetfutureSettlementType()) <<
		", Ticker: " << this->GetTicker() << ", Maturity: " << this->GetMaturityDate() << ", issued by: " << issue_country << endl;
}

BDFuture::BDFuture(string _productId, Bond _bd, string _ticker, date _maturityDate) : bd(_bd),
	Future(_productId, BondFuture, Physical, _ticker, _maturityDate) {
}
void BDFuture::print() {
	cout << this->ToString(this->GetfutureUnderlyingType()) << ", " << this->ToString(this->GetfutureSettlementType()) <<
		", Ticker: " << this->GetTicker() << ", Maturity: " << this->GetMaturityDate() << "Underlying Bond " << bd << endl;
}
